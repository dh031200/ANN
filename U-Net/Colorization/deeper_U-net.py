...
from tensorflow.keras.layers import Concatenate, Activation, Dropout, UpSampling2D
...
def main(epochs=100, batch_size=128, fig=True):
    ...
    if fig:
        plot_loss(history)
        plt.savefig('deeper_U-net_colorization_loss.png')
        plt.show()
        plt.clf()
        show_images(data, unet)
        plt.savefig('deeper_U-net_colorization_pred.png')
        plt.show()
