...
class AE(models.Model):
    def __init__(self, x_nodes=784, z_dim=None):
        ...
        x_d = layers.Dropout(rate=0.1)(x)   # changed
        h1 = layers.Dense(z_dim[0], activation='relu')(x_d)     # changed
...
def main():
    ...
    plot_Loss(history)
    plt.savefig('dsae_mnist.loss.png')
    plt.clf()
    plot_Accuracy(history)
    plt.savefig('dsae_mnist.acc.png')

    show_ae(autoencoder, X_test)
    plt.savefig('dsae_mnist.predicted.png')
    plt.show()
